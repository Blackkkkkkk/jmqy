...
<section>
	...
	<jsp:include page="/WEB-INF/layouts/left_menu.jsp"/>
		...
		<div class="position_all">
			<div class="position">您所在的位置：首页 -> 管理中心 -> 角色管理</div>
		</div>
		<div class="info">
			<form id="searchForm" action="${ctx}/transport/role/list" method="post">
...
function update(id){
  layer.open({
    ...
    area: ['550px','400px'],
    content: '${ctx}/transport/role/form?id='+id
  })
}
...
function authorize(id, roleName){
  parent.layer.open({
    ...
    area: ["750px", "500px"],
    content: '${ctx}/transport/role/authorize?id='+id
  })
}
...
function updateById(id, status){
    ...
    layer.confirm('真的'+typeStr+'这个角色么?', {icon: 3, title:'提示'}, function(index){
        $.ajax({
            type: "POST",
            url: "${ctx}/transport/role/updateById",
